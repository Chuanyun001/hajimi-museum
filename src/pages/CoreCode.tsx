...
import { coreCodeData, CoreCode } from '../data/coreCode';

export default function CoreCodePage() {
  const [selectedCode, setSelectedCode] = useState<CoreCode | null>(null);
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              <span className="text-[#569cd6]">核心代码</span>
              <span className="text-[#d4d4d4]">展区</span>
            </h1>
            <p className="text-[#6a9955] text-lg">
              // 猫咪的6大屎山核心代码，按 bug 严重程度排序
            </p>
            <p className="text-[#858585] text-sm mt-2">
              这些代码已经运行了千万年，从未重构过，注释全是乱码，但就是能跑
            </p>
          </div>

          {/* 代码卡片网格 */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {coreCodeData.map((code) => (
              <div
                key={code.id}
                className="bg-[#252526] rounded-lg border border-[#3c3c3c] overflow-hidden hover:border-[#569cd6] transition-all duration-300 cursor-pointer group"
              ...
              >
                ...
                <div className="px-4 py-2 bg-[#2d2d2d] border-t border-[#3c3c3c] opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <p className="text-xs text-[#569cd6]">
                    点击查看完整代码 →
                  </p>
                </div>
              </div>
